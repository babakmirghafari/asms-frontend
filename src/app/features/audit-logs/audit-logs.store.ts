...
export const AuditLogsStore = signalStore(
  withState(initialAuditLogsState),
  withComputed((store) => ({
    selected: computed(() =>
  ...
  withMethods((store) => ({
    // TODO(angular-logic-implementer): inject generated AuditLogsService and implement real API calls
    async load(): Promise<void> {
      patchState(store, { loading: true, error: null });
      // TODO(angular-logic-implementer): call service.listAuditLogss() and patchState with response
